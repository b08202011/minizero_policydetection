...
class Model:
    ...
    def load_model(self, model_file):
        ...
        if model_file:
            ...
            #self.network2 = torch.jit.load(f"{model_file.replace('.pkl','.pt')}", map_location=self.device)
...
class PolicyPlay():
    ...
    def calculate_similarity(self, candidate_embeds, query_embeds, topk = 4) : 
        ...
        compare_table =[] 
        '''
        #network_output = torch.reshape(candidate_embeds,(3,300,256))
        a = np.squeeze(np.asarray(candidate_embeds[0].cpu().detach(), dtype = np.float64))
        a1 = np.squeeze(np.asarray(candidate_embeds[1].cpu().detach(), dtype = np.float64)) 
        b = np.squeeze(np.asarray(candidate_embeds[400].cpu().detach(), dtype = np.float64)) 
        c = np.squeeze(np.asarray(candidate_embeds[700].cpu().detach(), dtype = np.float64))
        dists = 1 - dot(a, b) /(norm(a) * norm(b))
        print(dists)
        dists = 1 - dot(b, c) /(norm(b) * norm(c))
        print(dists)
        dists = 1 - dot(a, c) /(norm(a) * norm(c))
        print(dists)
        dists = 1 - dot(a, a1) /(norm(a) * norm(a1)) 
        print(dists) 
        '''
    ...
    def generate_games(self):
        ...
        # forward
            ...
            df = open('TestingDataset_Public/public_submission_template.csv').read().splitlines()
            games_id = [k.split(',',2)[0] for k in df]
            with open('test.csv','a') as f:
                answer_row = games_id[i+22340] + ',' + str(int(max_output)+1) + '\n'
