...
class Model:
    ...
    def load_model(self, training_dir, model_file):
        ...
        if model_file:
            ...
            self.network = torch.jit.load(f"{training_dir}/model/{model_file.replace('.pkl','.pt')}", map_location=self.device)
...
def train(model, training_dir, data_loader, val_loader,training_file,validation_file=None):
    ...
    #while(early_stop<10):
        ...
        if py.get_nn_type_name() == "alphazero":
            ...
            loss_policy += 0.01*calculate_loss2(network_output, label_policy[:, 0], label_value[:, 0], None, loss_scale) 
...
if __name__ == '__main__':
    ...
    model = Model()
    model_file = f'weight_iter_17640.pkl'
