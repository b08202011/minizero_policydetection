...
void DataLoaderThread::setAlphaZeroTrainingData(int batch_index)
{
    ...
    int env_id,pos;
    EnvironmentLoader env_loader;
    std::string PB_name,PW_name,player_name;
    while(true){
        int player_game_number = config::learner_batch_size/3;
        p = getSharedData()->replay_buffer_.sampleEnvAndPos();
        env_id = p.first, pos = p.second;
        env_loader = getSharedData()->replay_buffer_.env_loaders_[env_id];
        PB_name = env_loader.getTag("PB");
        PW_name = env_loader.getTag("PW");
        player_name = !PB_name.empty()? PB_name : PW_name;
        if((stoi(player_name)-1)%3 == batch_index/player_game_number ){break;}
        //std::cout<<"sample"<<std::endl;
    }

    int move_counts = env_loader.getActionPairs().size();
    //std::cout<<move_counts<<std::endl;
    pos = move_counts-1;
    p.second =pos;
    ...
    std::vector<float> zerovector(362,0);
    std::vector<float> features = env_loader.getFeatures(pos, rotation);
    std::vector<float> features2 = pos>1? env_loader.getFeatures(pos-1, rotation) : zerovector;
    std::vector<float> features3 = pos>2? env_loader.getFeatures(pos-2, rotation) : zerovector;
    ...
    switch (std::stoi(player_name))
    {
        case 1:
            //std::cout<<std::stoi(player_name)<<std::endl;
            player = {1,0,0};
            break;
        case 2:
            //std::cout<<std::stoi(player_name)<<std::endl;
    ...
    //std::cout<<player[0]<<player[1]<<player[2]<<std::endl;
    ...
    std::copy(player.begin(), player.end(), getSharedData()->getDataPtr()->player_name_ + player.size() * batch_index); 
    //add player name
}
...
{
    std::vector<float>  zerovector(18*19*19, 0);
    std::vector<float> features_;
    std::vector<float> features2_ = zerovector;
    std::vector<float> features3_ = zerovector; 
    ...
    for (int i = 0; i < move_counts; ++i) {
        ...
        if (i ==  move_counts-2){features2_ = env_.getFeatures();}
        if (i ==  move_counts-3){features3_ = env_.getFeatures();} 
        if (i ==  move_counts-1) {
            features_ = env_.getFeatures(); // (1,18*19*19)
            features_.insert(features_.end(),features2_.begin(),features2_.end());
            features_.insert(features_.end(),features3_.begin(),features3_.end());
