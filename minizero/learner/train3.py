...
def train(model, training_dir, data_loader, val_loader,training_file,validation_file=None):
    ...
    #while(early_stop<10):
        ...
        if py.get_nn_type_name() == "alphazero":
            ...
            network_output = model.network2(network_output)
            ...
            loss= calculate_style_loss(network_output, player_name[:,0],loss_scale)
            ...
            print('accuracy_style:',accuracy)
            loss_style.append(loss.item())
            accuracy_style.append(accuracy)
            ...
            for j in range(10):
                with torch.no_grad():
                    ...
                    if py.get_nn_type_name() == "alphazero":
                        ...
                        network_output = model.network2(network_output)
                        valloss= calculate_style_loss(network_output, player_name[:,0],loss_scale)
