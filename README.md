...
To train policy:
```bash
trainstyle.sh   #need to assign traindir(must exist a directory name model inside it) training config(can use load_training.cfg) training.sgf validation .sgf
                #need to modify the load model in minizero/learner/trainsimilarity.py
```
...
To evaluate :
```bash
styleeval.sh #need assign cfg(load_training.cfg) testing sgf training.sgf
        # need to modify the load model in policy_playstyle.py policy_playstylepri.py
